...
% Check to make sure that Pa + Pb is not > 1.        
        ...
        R(1:M,1:N) = 0.5;
% Generate an M by N array of uniformly distributed random numbers in the range   % (0,1). Then, Pa*(M*N) of them will have values <= a. The coordinates of these   % points we call 0 (pepper noise). Similarly, Pb*(M*N) points will have values in % the range > a & <= (a+b). These we call (salt noise).
        X = rand(M,N);
        c = find(X<=a);
        R(c) = 1;
        u = a + b;
    ...
    case 'exponential'
        ...
        if a <= 0
            error('the value of a must be positive for exponential operation')
        end
        k = -1/a;
        R = k*log(1 - rand(M,N));
    case 'erlang'
        ...
        if (b ~= round(b)| b <= 0)
            error('Parameter b should be a negative value for erlang')
        end
        k = -1/a;
        R = zeros(M,N);
        for j = 1:b
